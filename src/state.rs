...
#[derive(Clone, Debug)] 
pub struct FullState(pub [Cell;16]);
...
impl FullState {
    ...
    pub fn subBytes<T,F,i,j>(&mut self, mut f:F) -> [T;16] 
    where F: FnMut(&SBox)-> T
    {
        let a = f(&self.0[i][i*4 + j]);
        self.0[i][i*4 + j] = a;
        self
    }
   
   pub fn shiftRows<T,F>(&mut self, mut f:F) -> [T;16]
   where F: FnMut(&Row) -> T
   {
       let row = Row {
           cells: [self.0[0][0], self.0[1][0], self.0[2][0], self.0[3][0]],
           offset: 0,
       };
       let a = f(&row);
       self.0[0][0] = a;
   ...
   pub fn mixColumns<T,F>(&mut self, mut f:F) -> [T;16]
   where F: FnMut(&Column) -> T
   {
       let column = Column {
           cells: [self.0[0][0], self.0[1][0], self.0[2][0], self.0[3][0]],
           offset: 0,
